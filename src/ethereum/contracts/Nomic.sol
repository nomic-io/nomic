...
contract Nomic is ReentrancyGuard {
    ...
    function submitLogicCall(
        ...
        (bool success, bytes memory returnData) = address(this).call(
            abi.encodeWithSignature(
                "remoteCall((uint256[],address[],uint256[],address[],address,bytes,uint256,uint256,bytes32,uint256,address))",
                _args
            )
        );
        if (!success) {
    ...
    function remoteCall(LogicCallArgs memory _args) external {
