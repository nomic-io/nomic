...
fn multisig_script(pks: &[XOnlyPublicKey], threshold: u32, verify: bool) -> Result<Script> {
    if pks.is_empty() {
...
fn sort_keys(pks: &[XOnlyPublicKey]) -> Result<Vec<XOnlyPublicKey>> {
    if pks.len() < 2 {
        return Err(Error::Orga(orga::Error::App(
...
fn single_key_script(pk: XOnlyPublicKey, verify: bool) -> Script {
    let pk = pk.serialize().to_vec();
    if verify {
...
// TODO: move Babylon params into `Params` struct
...
fn unbonding_script(
    ...
    Ok(aggregate_scripts(&[
        ...
        multisig_script(covenant_keys, covenant_quorum, false)?,
    ]))
}
...
fn staking_script(
    ...
    let unbonding_script = unbonding_script(staker_key, covenant_keys, covenant_quorum)?;
    let slashing_script = slashing_script(staker_key, fp_keys, covenant_keys, covenant_quorum)?;
...
fn slashing_tx(
    ...
    slashing_addr: Address,
    slashing_min_fee: u64,
) -> Result<Transaction> {
    let staking_in = TxIn {
    ...
    let slashing_out = TxOut {
        ...
        script_pubkey: slashing_addr.script_pubkey(),
    };
    ...
    let change_out = TxOut {
        value: stake_value - slashing_value - slashing_min_fee,
...
fn unbonding_tx(
    ...
    let out_key = TaprootBuilder::new()
        ...
        .add_leaf(
            1,
            slashing_script(staker_key, fp_keys, covenant_keys, covenant_quorum)?,
        )?
        .finalize(&Secp256k1::new(), UNSPENDABLE_KEY.parse().unwrap())
        .unwrap()
        .output_key();
